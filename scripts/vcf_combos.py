...
def work(vcf_in, prefix):
    ...
    for path in results:
        ...
        if (path, alt_path) not in unique_pairs and (alt_path, path) not in unique_pairs:
            unique_pairs.append((path, alt_path))

    for index, (path, alt_path) in enumerate(unique_pairs):
        vcf1_out = VariantFile(f'{prefix}_{index}_A.vcf', 'w', header=vcf_in.header)
        vcf2_out = VariantFile(f'{prefix}_{index}_B.vcf', 'w', header=vcf_in.header)
